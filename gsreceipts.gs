...
function rcGetAllReceipts(email) {
  checkServerSession(email);
  return _rcGetRangeData('RANGERECEIPTS');
}

/**
 * Generate unique Trx ID (RTxxxxx)
 */
function rcGenerateTrxID(email) {
  checkServerSession(email);
 ...
 */
function rcSaveNewReceipt(rec, email) {
  // Session Check
  ...
  // Append row in correct column order (6 columns)
  sheet.appendRow([
  ...
  Logger.log('Receipt saved: ' + JSON.stringify(rec));
}

/**
 * Update existing receipt row by Trx ID
 */
function rcUpdateReceipt(rec, email) {
  // Session Check
  const session = checkServerSession(email);
  if (!session.active) {
    throw new Error("Sesi berakhir");
 ...
 */
function rcDeleteReceipt(trxID, email) {
  // Session Check
  const session = checkServerSession(email);
  if (!session.active) {
    throw new Error("Sesi berakhir");
